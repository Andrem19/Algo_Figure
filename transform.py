...
symbols = coins.oldCoins + coins.newCoins
times = [1, 5, 15]
for t in times:
    for s in symbols:
        file_list = glob.glob(f'newdata/futures/um/monthly/klines/{s}/{t}m/{s}-{t}m-*.csv')
        file_list.sort()
        ...
        df_final.to_csv(f'_crypto_data/{s}/{s}_{t}m.csv', header=False, index=False)
